...
void *producer(void *ptr) {
    ...
    for (i = 1; i <= MAX; i++) {
        ...
        */
        while (buffer != 0) {
            pthread_cond_wait(&condp, &the_mutex); //Questo meccanismo permette al producer, mentre detiene il lock, di rilasciare la risorsa e far sì che il consumatore possa compiere le sue operazioni, per poi rilasciare la risorsa.
        }
