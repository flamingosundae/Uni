...
void *producer(void *arg) {
	while (TRUE) {
		...
		up(&full);  // Segnala che c'è un posto pieno in più nel buffer
			    // E se chiamassi print_buffer DOPO l'uscita dalla regione critica? è prevedibile che al momento della stampa il buffer sarà GIÀ stato modificato dal consumer, essendo il producer uscito dalla 
			    // regione critica.
	}
}
